...
# some manually chosen values for the pydub silence threshold 
min_silence_len = 50 
silence_thresh = -30
...
def generate_syllables(timestamp_func, dataset_type, model_name):
  ...
  for index, row in csv.iterrows():
    ...
    # we'll need both the no space version 
    text_no_spaces = " " + row["unicode_no_space"]
    text_spaces = row["unicode"]
    # means that the original chinese contained a word + er compound
    if text_spaces == "BAD": # these are the lines that I separately marked as the words that have 儿 in them 
      print(f"known bad: {index}")
      continue

    # prepare the output for the syllables for this phrase
    ...
    # we have to use the get_tokens function instead of just passing in text as an argument to separate compound words 
    try:
    ...
    for i, interval in enumerate(timestamps):
      ...
      # cut the audio based on the timetsamps given by whisper 
      cut_audio(input_file=audio_path, output_file=output_path, start_time=(interval[0]+0.1)*1000, end_time=(interval[1]+0.08)*1000)
